...
final class PointCloud: UIViewController, ARSessionDelegate {
   ...
   // private let recordButton = UIButton()
    ...
    override func viewDidLoad() {
        ...
        pickFramesSlider.isHidden = true
        // UIButton
...
//        textLabel.textAlignment = .right
        ...
        let stackView = UIStackView(arrangedSubviews: [
            confidenceControl, rgbRadiusSlider, pickFramesSlider])
    ...
    @IBAction func tappedStartBtn(_ sender: UIButton){
        if (sender != recordButton) {
            return
        }
        updateIsRecording(_isRecording: !isRecording)
        sceneBtn.isEnabled = true
    }
    
    @IBAction func tappedSceneBtn(_ sender: UIButton){
        let resultVC = ResultViewController()
        resultVC.pointCloudString = renderer.getPointCloudData()
...
extension PointCloud: TaskDelegate {
    func didStartTask() {
        self.taskNum += 1
        updateTextLabel()
    }
...
extension PointCloud: MTKViewDelegate {
